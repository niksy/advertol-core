...
class ControlResolver {
	...
	destroy () {
		...
		this.controls.forEach(( control ) => {
			...
			const shouldTriggerControlCache = control._shouldTriggerControlCache;
			const onInitialControlTriggerCache = control._onInitialControlTriggerCache;

			this.zones.forEach(({ element, id, isEmpty }) => {

				/* eslint-disable no-undefined */
				const triggerPromise = Promise.all([
					Promise.resolve(shouldTriggerControlCache[id] || false),
					Promise.resolve(onInitialControlTriggerCache[id] || undefined)
				]).then(([isControlTriggered, initialControlTriggerResult]) => {
		...
		Promise.all(triggerPromises).then(() => {
